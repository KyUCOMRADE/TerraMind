// =========================
// Frontend: RegionCard.jsx (Updated)
// =========================
import React from 'react';

const getHealthStatus = healthIndex => {
  if (healthIndex >= 0.8) return { color:'#2E7D32', message:'Land is healthy 🌱', recommendation:'Maintain sustainable practices 🌿' };
  if (healthIndex >= 0.5) return { color:'#FBC02D', message:'Moderate health ⚠️', recommendation:'Monitor periodically 🌾' };
  if (healthIndex >= 0.3) return { color:'#F57C00', message:'Low health 🔧', recommendation:'Consider soil restoration 🌍' };
...
export default function RegionCard({ selectedRegion }) {
  ...
  return (
    <div style={{ border:`2px solid ${color}`, borderRadius:'10px', padding:'15px', maxWidth:'400px', backgroundColor:'#f0fff0', boxShadow:'2px 2px 12px rgba(0,0,0,0.1)', marginTop:'20px' }}>
      ...
      <p style={{ marginTop:'5px', fontStyle:'italic', color }}>Recommendation: {aiRecommendation || recommendation}</p>
    </div>
  );
}
